...
export default function ProductPage() {
  ...
  // API product state
  const [apiProduct, setApiProduct] = React.useState<Product | null>(null);
  const [isLoadingApi, setIsLoadingApi] = React.useState<boolean>(false);

  // Synchronize/fetch product when slug changes
  React.useEffect(() => {
        ...
        .then((data) => {
          ...
          if (apiProd) {
            setApiProduct({
              ...
              description: apiProd.description || "High quality API product.",
              ...
              specs: [{ name: "Category", value: apiProd.category || "General" }],
              vendor: "API Partner"
            });
          }
        })
  ...
  const isLoading = !localProduct && slug.startsWith("api-") && (isLoadingApi || !apiProduct);
  ...
  const isWishlisted = mounted && product ? wishlist.some((p) => p.id === product.id) : false;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
            ...
            <div className="space-y-6">
              <ProductGallery 
                ...
                name={product.name} 
                isApiProduct={String(product.id).startsWith("api-")}
              />
            </div>
